...
void custom_handler(int signal_num){
    ...
    for(j=0; j < clients_count; j++){
        ...
        msgsnd(id_server, &msg_smpl, sizeof(msg_smpl.payload), msg_smpl.type);
        sleep(5);
...
int main(int argc, char* argv[]) {
    ...
    while (1) {
        ...
        msgrcv(id_server, &msg_srv, sizeof(msg_srv.pid), 1, 0);
        printf("Message from client with key %ld\n", msg_srv.type);
        
        is_client = is_already_client(msg_srv.pid, clients);
        if (is_client == 0){
            clients[clients_count] = msg_srv.pid;
        ...
        for(j=0; j < clients_count; j++){
            ...
            printf("%lu\n", clients[j]);
            char str[20];
            sprintf(str, "Message from %lu\n", msg_smpl.type);
            strcpy((char*)&msg_smpl.payload, str);
            printf("Queue id to send is %d\n", id_server);
            msgsnd(id_server, &msg_smpl, sizeof(msg_smpl.payload), msg_smpl.type);
