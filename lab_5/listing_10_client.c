...
int main(int argc, char* argv[]) {
    ...
    key_server = ftok("/home/vboxuser/mvkosovy/labs_aos_c/lab_5/listing_10_server", 2);
    printf("%d\n", key);
    printf("%d\n", key_server);
    ...
    //strcpy((char*)&msg_request.payload, "Ping\n");
    
    for(int i = 0; i < 100; i++) {
    	
        msgsnd(id_server, &msg_request, sizeof(msg_request.pid), msg_request.type);
        printf("Client sent a request\n");
        memset(&msg_answer, 0, sizeof(msg_answer));
        msgrcv(id_server, &msg_answer, sizeof(msg_answer.payload), pid, 0);
        
        printf("Answer for client id %lu: %s\n", msg_answer.type, msg_answer.payload);
        sleep(1);
