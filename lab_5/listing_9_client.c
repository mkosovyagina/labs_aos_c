...
int main(int argc, char* argv[]) {
    ...
    key_server = ftok("/home/vboxuser/mvkosovy/labs_aos_c/lab_5/listing_9_server", 2);
    printf("%d\n", key);
    printf("%d\n", key_server);
    ...
    msg_request.type = pid;
    strcpy((char*)&msg_request.payload, "Ping\n");
    
    for(int i = 0; i < 100; i++) {
    	
        msgsnd(id_server, &msg_request, sizeof(msg_request.payload), msg_request.type);
        printf("Client sent a request\n");
        memset(&msg_answer, 0, sizeof(msg_answer));
        msgrcv(id_client, &msg_answer, sizeof(msg_answer.payload), msg_request.type, 0);
